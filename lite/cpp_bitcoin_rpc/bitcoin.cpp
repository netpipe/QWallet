...
namespace bitcoin {  
    ...
    address_info client::validateaddress( const std::string& address )
    {
        ...
        ai.scriptPubKey = pt.get<std::string>("result.scriptPubKey");
        return ai;
    }
    ...
    std::string client::createrawtransaction( const std::string& transaction_input, const std::string& address_amount )
    {
        std::stringstream ss;
        ss << "{\"jsonrpc\": \"1.0\", \"id\":\"1\", \"method\": \"createrawtransaction\", \"params\": [";
    ...
    bool client::signrawtransaction( const std::string& transaction_input)
    {
        std::stringstream ss;
        ...
        ss << "] }";
        boost::property_tree::ptree pt =  my->request(ss.str());
        return pt.get<std::string>("result.complete")=="true";
    }

    std::string client::sendrawtransaction( const std::string& transaction_input )
    {
        std::stringstream ss;
